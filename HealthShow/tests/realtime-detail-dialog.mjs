...
const runtimeSource = readFileSync('src/views/health-monitor/real-time/realtime-runtime.js', 'utf8')
...
test('real-time page exposes reliable refresh and server-side paging controls', () => {
  ...
  assert.match(runtimeSource, /page: this\.currentPage/, 'runtime should request the active server page')
  assert.match(runtimeSource, /status: this\.searchForm\.status/, 'status filter should be sent to the server')
