 ...
 * 后端重启瞬间：
 *   - 内存中的 Token 数据全部丢失
 *   - 前端携带 Token 请求任何接口 → 后端返回 5xx（服务还没完全启动）
 *     或 500（无法识别 Token，内部异常）
 *
 * 代码逻辑：HTTP 5xx + 有 Token = 可能是后端重启/Token 失效
 *   → 静默跳转登录，不弹错误提示（避免用户看到"服务器错误"这种技术性提示）
 */
...
service.interceptors.response.use(
  ...
  error => {
    ...
    let message = '请求失败'
    let isTokenIssue = false  // 是否可能是 Token 失效（后端重启等）
    ...
    if (error.response) {
      ...
      switch (status) {
        ...
        case 404:
          ...
          break
        case 500:
          // HTTP 500 内部服务器错误 + 有 Token = 可能是后端重启/Token 失效
          // 策略：静默跳登录，而不是弹"服务器错误"（对用户更友好）
          if (getToken()) {
            isTokenIssue = true
          } else {
            message = '服务器错误'
