...
public class DashboardDepartmentQueryService {
    ...
    public List<DeptHealthCountView> getDeptHealthCounts(String startTime, String endTime) {
        ...
        List<DeptHealthCountView> hit = deptStatsCache.getIfFresh(cacheKey);
        if (hit != null) {
            return hit;
        }
        ...
        deptStatsCache.put(cacheKey, result, DEPT_STATS_TTL);
        return result;
    }
    ...
    public List<DeptPersonStatView> getDeptPersonStats(String startTime, String endTime) {
        String cacheKey = HealthCacheKeys.key(startTime, endTime);
        List<DeptPersonStatView> hit = deptPersonStatsCache.getIfFresh(cacheKey);
        if (hit != null) {
            return hit;
        }
        String[] range = range30(startTime, endTime);
        ...
        List<DeptPersonStatView> result = toDeptPersonStatViews(
                dashboardDepartmentMapper.getDeptPersonStatsDirect(hSrc, wSrc, range[0], range[1])
        );
        deptPersonStatsCache.put(cacheKey, result, DEPT_PERSON_STATS_TTL);
        return result;
    }
    ...
    public List<DeptHealthComparisonView> getDeptHealthComparison(int days) {
        ...
        List<DeptHealthComparisonView> hit = deptHealthComparisonCache.getIfFresh(cacheKey);
        if (hit != null) {
            return hit;
        }
        ...
        List<DeptHealthComparisonView> result = toDeptHealthComparisonViews(
                dashboardDepartmentMapper.getDeptHealthComparisonDirect(tblSrc, days)
        );
        deptHealthComparisonCache.put(cacheKey, result, DEPT_HEALTH_COMP_TTL);
        return result;
    }
