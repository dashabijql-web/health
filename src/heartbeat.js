 ...
 *   - 接口失败（401/500/网络错误）→ Token 已失效，静默清除 Token 并跳转登录页
 *   - 用户无感知（没有弹窗），最多等待30秒就会自动跳转到登录页
 *
 * 【visibilitychange 事件】
 *
 ...
 * 这意味着：即使这个请求失败（401、500、网络错误），也不会弹出 ElMessage 错误提示。
 * 用户只会看到页面自动跳转到登录页，没有任何弹窗打扰。
 *
 * 【模块化设计】
 *
...
async function ping() {
  ...
  try {
    ...
    // 请求失败：Token 可能已过期（后端重启、超时等）
    // request.js 已静默处理（不弹窗），这里只做清理+跳转

    // 清除本地 Token（Cookie）和 Vuex store 中的用户状态
    await store.dispatch('user/resetToken')

    // 跳转到登录页（replace 替换历史记录，不产生额外的浏览器历史条目）
